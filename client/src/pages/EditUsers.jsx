...
export default function EditUsers(data) {
  ...
  const user_id = data.location.state
  useEffect(() => {
    axios.get(`${REACT_APP_API_URL}/${user_id}`).then((res) => {
      const user = res.data
      setUser(user.data)
    })
  }, [user_id])
  ...
  function updateUser(e) {
    ...
    axios
      .put(`${REACT_APP_API_URL}/${user._id}`, user)
      .then(() => {
  ...
  return (
    <div>
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
        <h2 style={{ textAlign: 'center', marginBottom: 15 }}>Edit User</h2>
        <Form.Row>
